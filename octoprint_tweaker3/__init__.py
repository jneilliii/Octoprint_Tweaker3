...
class tweaker3(octoprint.plugin.RestartNeedingPlugin):
    ...
    def tweak_on_upload(self, path, file_object, links=None, printer_profile=None, allow_overwrite=True, *args,
                        **kwargs):
        name, my_type = os.path.splitext(file_object.filename)
        if not ((my_type == "stl") or (my_type == "3mf")):
            return file_object

        myFileHandler = FileHandler()
        try:
            objs = myFileHandler.load_mesh(path)
            if objs is None:
                sys.exit()
        ...
        if not args.result:
            try:
                FileHandler.write_mesh(objs, info, path,
...
def __plugin_load__():
    ...
    __plugin_hooks__ = {
        "octoprint.filemanager.preprocessor": __plugin_implementation__.tweak_on_upload,
